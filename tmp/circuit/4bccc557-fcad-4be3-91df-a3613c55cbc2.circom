...
template Email(msg_bytes) {
	...
	for (var i = 0; i < num_bytes; i++) {
		...
		states[i+1][22] <== multi_or[0][i].out;
		eq[14][i] = IsEqual();
		eq[14][i].in[0] <== in[i];
		eq[14][i].in[1] <== 45;
		and[23][i] = AND();
	...
	// substrings calculated: [{(21, 22), (22, 23), (23, 24), (24, 25), (25, 26), (26, 27), (27, 28), (28, 29), (29, 30), (30, 31), (31, 22)}]
	signal prev_states0[11][msg_bytes];
	signal is_substr0[msg_bytes];
	signal is_reveal0[msg_bytes];
	signal output reveal0[msg_bytes];
	for (var i = 0; i < msg_bytes; i++) {
		 // the 0-th substring transitions: [(21, 22), (22, 23), (23, 24), (24, 25), (25, 26), (26, 27), (27, 28), (28, 29), (29, 30), (30, 31), (31, 22)]
		...
		prev_states0[3][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][24];
		prev_states0[4][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][25];
		prev_states0[5][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][26];
		prev_states0[6][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][27];
		prev_states0[7][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][28];
		prev_states0[8][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][29];
		prev_states0[9][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][30];
		prev_states0[10][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][31];
		is_substr0[i] <== MultiOR(11)([prev_states0[0][i] * states[i+2][22], prev_states0[1][i] * states[i+2][23], prev_states0[2][i] * states[i+2][24], prev_states0[3][i] * states[i+2][25], prev_states0[4][i] * states[i+2][26], prev_states0[5][i] * states[i+2][27], prev_states0[6][i] * states[i+2][28], prev_states0[7][i] * states[i+2][29], prev_states0[8][i] * states[i+2][30], prev_states0[9][i] * states[i+2][31], prev_states0[10][i] * states[i+2][22]]);
		is_reveal0[i] <== MultiAND(3)([out, is_substr0[i], is_consecutive[i][2]]);
		reveal0[i] <== in[i+1] * is_reveal0[i];
	}
}
component main = Email(1024, 1024, 121, 17, 7);